...
def main():
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(combined_dataset)):
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if mean_squared < minimum:
                minimum = mean_squared
            if mean_squared > maximum:
                maximum = mean_squared

        # Saving model of specific fold
        # torch.save(network.state_dict(), f'./results/model-fold-{fold}.pth')

        # Save results to csv file
        total /= n_epochs
        dic[fold] = [total, minimum, maximum]
...
if __name__ == '__main__':
    ...
    with open('results/results_barchart_kfold.csv', mode='w') as file:
        file_writer = csv.writer(file)

        file_writer.writerow(['FOLDS', 'MSE', 'MINIMUM', 'MAXIMUM'])

        for fold in data.keys():
            file_writer.writerow([fold, data[fold][0], data[fold][1], data[fold][2]])

...
def main():
    ...
    for bs in batch_sizes:
        ...
        while cont:
            ...
            train(epoch)
            mean_squared = test(test_loader)
            process = time.time() - start
            arr.append([epoch, bs, mean_squared,  process])
            epoch += 1

            if mean_squared > prev_mean and epoch > 2:
                cont = False
            else:
                prev_mean = mean_squared
...
if __name__ == '__main__':
    ...
    with open('results/stopping_criterion.csv', mode='w') as file:
        file_writer = csv.writer(file)

        file_writer.writerow(['EPOCH', 'BS', 'MSE', 'TIME'])

        for ep in data:
            file_writer.writerow([ep[0], ep[1], ep[2], ep[3]])
